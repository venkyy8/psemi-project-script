	...
	public class ScriptManager 
	{
		...
		private void CreateTransaction(Script script, XElement item)
		{
			...
			Transaction tranaction = new Transaction
			{
				...
				RegisterAddress = Convert.ToByte(item.Attribute("RegisterAddress").Value, 16),
				Data = bData.ToArray(),

				// Optional
				RepeatCount = (int)repeatCount,
				InnerDelayMs = (ushort)innerDelayMs,
				Length = (sData == null) ? (byte)(Convert.ToInt32(item.Attribute("Length").Value) & 0xFF) : (byte)(sData.Length & 0xFF),
			};
