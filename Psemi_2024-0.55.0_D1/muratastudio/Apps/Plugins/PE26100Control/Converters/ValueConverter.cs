...
namespace PE26100Control.Converters
{
	public class ValueConverter : IValueConverter
	{
		public object Convert(object value, Type targetType, object parameter, System.Globalization.CultureInfo culture)
		{
			...
			if(!String.IsNullOrEmpty(lo.MinimumRange) && !String.IsNullOrEmpty(lo.MaximumRange))
			{
				if ((newVal > int.Parse(lo.MinimumRange)) && (newVal < int.Parse(lo.MaximumRange)))
				{
					newVal = 235;
				}
			}
			else if (!String.IsNullOrEmpty(lo.MaximumRange))
			{
				if (newVal > int.Parse(lo.MaximumRange))
				{
					newVal = 255;
				}
			}
