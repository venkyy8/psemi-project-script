...
{
	internal class PE24106R01 : CustomizeBase
	{
		...
		public PE24106R01(Device device)
			: base(device)
		{
		}

        public PE24106R01(Device device, bool isDemo = false)
			: base(device, isDemo)
		{
			if (IsDemo)
			{
		...
		public override void ModifyDeviceConfig(object obj1, object obj2)
		{
			if ((string)obj1=="1")
            {
                ModifyTransformAttribute(true, obj2);
            }
            else
            {
                ModifyTransformAttribute(false, obj2);
            }

        }


		private void ModifyTransformAttribute(bool bitValue, object obj)
		{
			int selectedConfig = (int)obj -1;

			string[] panels = { "Config1Panels", "Config2Panels", "Config3Panels" };
			string[] bucks1 = { "Buck1 Control" };
			string[] bucks2 = { "Buck1 Control", "Buck2 Control" };

			string formulaOnBit0 = "((((0.1/4095) * x)/.01)*2)/0.001";
			string formulaOnBit1 = "((((0.15/4095) * x)/.01)*2)/0.001";

			string formula = "";
			
			if (bitValue == false)
			{
				formula = formulaOnBit0;
			}
			else
			{
				formula = formulaOnBit1;
			}				

			//Iout1 and- Iout2
			//IOUTP1_LSB|IOUTP1_MSB  0x4A 0x4B
			//IOUTP2_LSB|IOUTP2_MSB  0x4C 0x4D 

				string[] bucks;
				if (panels[selectedConfig] == "Config2Panels")
				{
					bucks = bucks2;
				}
				else
				...
				foreach (string buck in bucks)
				{
					var uiControl = Device?.UiElements?.Element("" + panels[selectedConfig] + "")
										.Elements("Panel").FirstOrDefault(a => a.Attribute("Name").Value == "" + buck + "")
										.Elements("FormattedDisplay").FirstOrDefault(a => a.Attribute("Unit").Value == "mA");
					if (uiControl != null)
					{
						uiControl.Attribute("Transform").Value = formula;
                    }
				}
