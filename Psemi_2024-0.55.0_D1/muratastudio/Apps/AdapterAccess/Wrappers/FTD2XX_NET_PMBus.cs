...
**  1.0.15		Modifying driver for PMBus
*/
using System;
...
namespace FTD2XX_NET_PMBus
{
	...
	public class FTDI
	{
		...
		{
			// Set whether PEC is used
			_pec = config.IsPEC ? 1 : 0;
		...
		/// <param name="command">The starting address of the register to begin writing data.</param>
		...
		/// <returns>Status</returns>
		public FT_STATUS PMBus_DeviceWrite(byte targetAddress, byte? command, uint length, ref byte[] data, ref uint bytesTransfered)
		{
			...
			int counter = 0;
			while (counter++ < _tryCount)
			{
				ftStatus = FT_STATUS.FT_OK;
				ftStatusError = FT_STATUS.FT_OK;
				...
				if (isHighSpeed)
				{
					...
					ftStatus = SetPMBusBitRate(fastSpeedClock);
					if (ftStatus != FT_STATUS.FT_OK)
						ftStatusError = ftStatus;
				}
				else
				{
					...
					ftStatus = SetPMBusBitRate(clockRate);
					if (ftStatus != FT_STATUS.FT_OK)
						ftStatusError = ftStatus;
				}

				// Start Bit
				ftStatus = SendPMBusStartBit();
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;
				...
				{
					// Send Master Code
					ftStatus = SendPMBusData(masterCode, AcknowledgeType.NACK);
					if (ftStatus != FT_STATUS.FT_OK)
						ftStatusError = ftStatus;
				...
				// Send Target Address
				ftStatus = SendPMBusData((byte)((targetAddress << 1) & 0xFE));
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;
                ...
                if (command.HasValue)
                {
                    ...
                    ftStatus = SendPMBusData(command.Value);
                    if (ftStatus != FT_STATUS.FT_OK)
                        ftStatusError = ftStatus;
                }
				...
				if (ftStatusError == FT_STATUS.FT_OK)
				{
					break;
				}
			}
		...
		/// </summary>
		/// <param name="targetAddress">The true address of the I2C device. The shift is done internally.</param>
		/// <param name="command">The starting address of the register to begin reading data.</param>
		/// <param name="length">Number of bytes to read.</param>
		/// <param name="data">Buffer for read data bytes.</param>
		/// <param name="bytesTransfered">The number of bytes that were transfered.</param>
		/// <returns>Status</returns>
		public FT_STATUS PMBus_DeviceRead(byte targetAddress, ushort command, uint length, ref byte[] data, ref uint bytesTransfered)
		{
            //byte msb = (byte)(command >> 8);
            //byte lsb = (byte)(command & 0xFF);
            //length = (uint)((length == 1) ? 2 : length);
			...
			while (counter++ < _tryCount)
			{
				...
				// Command Address
				ftStatus = SendPMBusData((byte)command);
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;

                //// Command Address
                //ftStatus = SendPMBusData(msb);
                //if (ftStatus != FT_STATUS.FT_OK)
                //    ftStatusError = ftStatus;
				...
				// Send Target Address
				ftStatus = SendPMBusData((byte)((targetAddress << 1) | 0x01));
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;
		...
		{
			gpioData = value;
			return SetPMBusLinesIdle();
		}

		private FT_STATUS SetPMBusBitRate(ushort clockDivisor)
		{
		...
		/// </summary>
		/// <returns>Status</returns>
		private FT_STATUS SetPMBusLinesIdle()
		{
		...
		/// </summary>
		/// <returns>Status</returns>
		private FT_STATUS SendPMBusStartBit()
		{
			dwNumBytesToSend = 0;
		...
		/// <returns>Status</returns>
		private FT_STATUS SendPMBusData(byte value, AcknowledgeType type = AcknowledgeType.ACK)
		{
			// Initialize ftStatus to something other than FT_OK
			FT_STATUS ftStatus = FT_STATUS.FT_OTHER_ERROR;
		...
		/// </summary>
		/// <returns>Status</returns>
		private FT_STATUS SendPMBusStopBit()
		{
			dwNumBytesToSend = 0;
			pinData = (byte)(gpioData | 0x0C);
