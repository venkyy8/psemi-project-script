	...
	public class FTDI
	{
		...
		public FTDI()
		{
			...
			else
			{
				...
				throw new FT_EXCEPTION("Failed to load FTD2XX.DLL.  Are the FTDI drivers installed?");
			}
		}
		...
		public enum FT_STATUS
		{
			...
			FT_OTHER_ERROR,
			/// <summary>
			/// I2C no ack from slave
			/// </summary>
			FT_I2C_NO_ACK,
			/// <summary>
			/// I2C no nack from slave
			/// </summary>
			FT_I2C_NO_NACK,
			/// <summary>
		...
		private const int READ_TIMEOUT_DELAY = 1;
		private const int READ_TIMEOUT_COUNTER = 500;

		private const byte MSB_FALLING_EDGE_CLOCK_BYTE_OUT = 0x11;
		private const byte MSB_RISING_EDGE_CLOCK_BIT_IN = 0x22;
		private const byte CLK_DIV_5_OFF = 0x8A;
		private const byte CLK_DIV_5_ON = 0x8B;

		private byte clockDivider = CLK_DIV_5_OFF;		// 60MHz Clock
		private bool isHighSpeed = false;
		private ushort fastSpeedClock = 0x3C;
		private ushort clockRate = 0x3C;
		private byte masterCode = 0x09;

		private uint dwNumInputBuffer = 0;				// Number of bytes which we want to read
		private uint dwNumBytesRead = 0;				// Number of bytes actually read
		private uint ReadTimeoutCounter = 0;			// Used as a software timeout counter when the code checks the Queue Status

		private uint dwNumBytesToSend = 0;				// Counter used to hold number of bytes to be sent
		private uint dwNumBytesSent = 0;				// Holds number of bytes actually sent (returned by the read function)
		
		private byte[] InputBuffer = new byte[1024];	// Buffer to hold MPSSE commands and data to be sent to FT232H
		private byte[] OutputBuffer = new byte[1024];	// Buffer to hold Data bytes read from FT232H

		private byte pinData = 0;
		private byte gpioData = 0x80;					// AD7 - AD0 0000 0111
		// AD4 - GPIOL0 - Grey
		// AD5 - GPIOL1 - Purple
		// AD6 - GPIOL2 - White
		// AD7 - GPIOL3 - Blue
		...
		public class MpsseConfiguration
		{
			private DriveByZero _driveByZero = DriveByZero.BOTH_SCL_SDA;
			public int ClockSelection { get; set; }
			public bool IsThreePhaseClocking { get; set; }
			public byte MasterCode { get; set; }
			public DriveByZero DriveByZero
			{
				get { return _driveByZero; }
				set { _driveByZero = value; }
			}
			public byte GetPhaseClocking()
			{
				if (IsThreePhaseClocking)
					return 0x8C;
				else
					return 0x8D;
			}
		...
		#region MPSSE_METHODS

		/// <summary>
		/// Configures the MPSSE.
		/// </summary>
		/// <returns>Status</returns>
		public FT_STATUS InitializeMpsse(MpsseConfiguration config)
		{
			// Set the master code byte
			masterCode = config.MasterCode;

			// Setup the bit-rate based on 3 phase clocking and configure flags if a high speed rate is 
			// selected. Must also ensure the 400KHz is fixed for sending the high speed pre-emble byte.
			// See method for details.
			SetupClockForMpsse(config);

			// #########################################################################################
			// Initialize the MPSSE
			// #########################################################################################

			// Initialize ftStatus to something other than FT_OK
			FT_STATUS ftStatus = FT_STATUS.FT_OTHER_ERROR;

			ftStatus = ResetDevice();
			ftStatus |= Purge(FT_PURGE.FT_PURGE_RX | FT_PURGE.FT_PURGE_TX);

			ftStatus |= InTransferSize(65535);					// Set USB request transfer sizes
			ftStatus |= SetCharacters(0, false, 0, false);		// Disable event and error characters
			ftStatus |= SetTimeouts(3000, 3000);				// Set the read and write timeouts to 3 seconds
			ftStatus |= SetLatency(16);							// Keep the latency timer at default of 16ms
			ftStatus |= SetBitMode(0, 0);						// Reset the mode to whatever is set in EEPROM
			ftStatus |= SetBitMode(0, 2);						// Enable MPSSE mode
			...
			ftStatus = Write(OutputBuffer, dwNumBytesToSend, ref dwNumBytesSent);	// Send off the invalid command
			...
			ftStatus = ftStatus = GetRxBytesAvailable(ref dwNumInputBuffer);

			while ((dwNumInputBuffer < 2) && (ftStatus == FT_STATUS.FT_OK) && (ReadTimeoutCounter < 500))
			{
				...
				ftStatus = ftStatus = GetRxBytesAvailable(ref dwNumInputBuffer);	// Get number of bytes in the input buffer
				ReadTimeoutCounter++;
			...
			if ((ftStatus == FT_STATUS.FT_OK) && (ReadTimeoutCounter < 500))
			{
				ftStatus = Read(InputBuffer, dwNumInputBuffer, ref dwNumBytesRead); // 'Readout the data from input buffer'
			...
			dwNumBytesToSend = 0;											// Clear the output buffer
																			// 0x8A - Disable clock divide-by-5 for 60Mhz master clock
			OutputBuffer[dwNumBytesToSend++] = clockDivider;				// 0x8B - Enable clock divide by 5 for 12MHz master clock
			OutputBuffer[dwNumBytesToSend++] = 0x97;						// Turn off adaptive clocking
																			// 0x8D = Disables 3 phase data clocking
			OutputBuffer[dwNumBytesToSend++] = config.GetPhaseClocking();	// 0x8C = Enable 3 phase data clocking, data valid on both clock edges for I2C

			OutputBuffer[dwNumBytesToSend++] = 0x9E;						// Enable drive-zero mode on the lines used for I2C ...
			OutputBuffer[dwNumBytesToSend++] = (byte)config.DriveByZero;	// on the bits AD0, 1 and 2 of the lower port...
			...
			// Send off commands
			ftStatus = Write(OutputBuffer, dwNumBytesToSend, ref dwNumBytesSent);

			Thread.Sleep(20);

			// Set line idle
			ftStatus = SetI2cLinesIdle();
		...
		/// <summary>
		/// Writes one or many bytes to an I2C slave device.
		/// </summary>
		/// <param name="targetAddress">The true address of the I2C device. The shift is done internally.</param>
		/// <param name="registerAddress">The starting address of the register to begin writing data.</param>
		/// <param name="length">Number of bytes to write.</param>
		/// <param name="data">An array of data bytes.</param>
		/// <param name="bytesTransfered">The number of bytes that were transfered.</param>
		/// <returns>Status</returns>
		public FT_STATUS I2C_DeviceWrite(byte targetAddress, byte registerAddress, uint length, ref byte[] data, ref uint bytesTransfered)
		{
			// Initialize ftStatuses
			FT_STATUS ftStatus = FT_STATUS.FT_OK;

			// Saves the last error
			FT_STATUS ftStatusError = FT_STATUS.FT_OK;

			// Bus Idle
			ftStatus = SetI2cLinesIdle();
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;

			// If High-Speed is enabled then we need to set the clock rate to 400KHz
			// to send the master code before switching to high speed clock.
			if (isHighSpeed)
			{
				// Set Bit Rate to 400KHz
				ftStatus = SetI2cBitRate(fastSpeedClock);
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;
			}
			else
			{
				// Otherwise, ensure the clock rate is set to the selected rate
				ftStatus = SetI2cBitRate(clockRate);
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;
			}

			// Start Bit
			ftStatus = SendI2cStartBit();
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;
			...
			{
				// Send Master Code
				ftStatus = SendI2cData(masterCode, AcknowledgeType.NACK);
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;
			...
			// Send Target Address
			ftStatus = SendI2cData((byte)((targetAddress << 1) & 0xFE));
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;

			// Send Register Address
			ftStatus = SendI2cData(registerAddress);
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;
			...
			// Stop Bit
			ftStatus = SendI2cStopBit();
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;

			// Return transaction status
			return (ftStatusError != ftStatus) ? ftStatusError : ftStatus;
		}


		public FT_STATUS I2C_DeviceWrite(byte targetAddress, ushort registerAddress, uint length, ref byte[] data, ref uint bytesTransfered)
		{
			...
			// Regsiter Address
			// Splitting the address into lower byte and- upper byte
			var lowerByte = (byte)(registerAddress & 0xff);
			var upperByte = (byte)(registerAddress >> 8);

			//Send upper byte
			ftStatus = SendI2cData(upperByte);
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;

			//Send lower byte
			ftStatus = SendI2cData(lowerByte);
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;
		...
		/// <summary>
		/// Reads one or many bytes from an I2C slave device.
		/// </summary>
		/// <param name="targetAddress">The true address of the I2C device. The shift is done internally.</param>
		/// <param name="registerAddress">The starting address of the register to begin reading data.</param>
		/// <param name="length">Number of bytes to read.</param>
		/// <param name="data">Buffer for read data bytes.</param>
		/// <param name="bytesTransfered">The number of bytes that were transfered.</param>
		/// <returns>Status</returns>
		public FT_STATUS I2C_DeviceRead(byte targetAddress, byte registerAddress, uint length, ref byte[] data, ref uint bytesTransfered)
		{
			// Initialize ftStatuses
			FT_STATUS ftStatus = FT_STATUS.FT_OK;
			...
			// Send Target Address
			ftStatus = SendI2cData((byte)((targetAddress << 1) | 0x01));
			if (ftStatus != FT_STATUS.FT_OK)
				ftStatusError = ftStatus;

			bytesTransfered = 0;
			// Send Clock and read data
			for (int i = 0; i < length; i++)
			{
				if (i < length - 1)
					ftStatus = ReadByteAndSendACK(out data[i]);
				else
					ftStatus = ReadByteAndSendNAK(out data[i]);
				bytesTransfered++;
				if (ftStatus != FT_STATUS.FT_OK)
					ftStatusError = ftStatus;
			}
		...
		public FT_STATUS I2C_DeviceRead(byte targetAddress, ushort registerAddress, uint length, ref byte[] data, ref uint bytesTransfered)
		{
			//throw new NotImplementedException();
			...
			//else
   //         {
			//	ftStatus = SendI2cData(Convert.ToByte(registerAddress));
			//	if (ftStatus != FT_STATUS.FT_OK)
			//		ftStatusError = ftStatus;
			//}
		...
		public FT_STATUS SetGpio(byte value)
		{
			gpioData = value;
			return SetI2cLinesIdle();
		}

		private FT_STATUS SetI2cBitRate(ushort clockDivisor)
		{
			// With the clock divide by 5 turned ON, the SK clock frequency can be found as:
			// SK frequency = 12MHz /((1 +0xValueH*256) OR 0xValueL])*2)
			dwNumBytesToSend = 0;													// 'Clear the output buffer'
			OutputBuffer[dwNumBytesToSend++] = 0x86;								// Command to set clock divisor
			OutputBuffer[dwNumBytesToSend++] = (byte)(clockDivisor & 0xFF);			// Set 0xValueL of clock divisor
			OutputBuffer[dwNumBytesToSend++] = (byte)((clockDivisor >> 8) & 0xFF);	// Set 0xValueH of clock divisor

			// Send off commands
			return Write(OutputBuffer, dwNumBytesToSend, ref dwNumBytesSent);
		}
		...
		/// </summary>
		/// <returns>Status</returns>
		private FT_STATUS SetI2cLinesIdle()
		{
			dwNumBytesToSend = 0;			//Clear output buffer
			pinData = (byte)(gpioData | 0x0F);
			// Repeat commands to ensure the minimum period of the start hold time is achieved
			for (int i = 0; i < 4; i++)
			{
				OutputBuffer[dwNumBytesToSend++] = 0x80;	// Command to set ADbus direction/ data
				OutputBuffer[dwNumBytesToSend++] = pinData; // Bring data out high
		...
		/// </summary>
		/// <returns>Status</returns>
		private FT_STATUS SendI2cStartBit()
		{
			dwNumBytesToSend = 0;
			pinData = (byte)(gpioData | 0x0D);
			// Pull Data line low, leaving clock high (open-drain)
			// Repeat commands to ensure the minimum period of the start hold time is achieved
			for (int i = 0; i < 10; i++)
			{
				OutputBuffer[dwNumBytesToSend++] = 0x80;	// 'Command to set ADbus direction/ data
				OutputBuffer[dwNumBytesToSend++] = pinData; // 'Bring data out low (bit 1) 
				OutputBuffer[dwNumBytesToSend++] = 0xFB;	// 'Set pins o/p except bit 2 (data_in)
			}

			Write(OutputBuffer, dwNumBytesToSend, ref dwNumBytesSent);

			pinData = (byte)(gpioData | 0x0C);
			// Pull Clock line low now, making both clock and data low
			// Repeat commands to ensure the minimum period of the start hold time is achieved
			for (int i = 0; i < 20; i++)
			{
				OutputBuffer[dwNumBytesToSend++] = 0x80;	// 'Command to set ADbus direction/ data
				OutputBuffer[dwNumBytesToSend++] = pinData; // 'Bring clock line low too 
				OutputBuffer[dwNumBytesToSend++] = 0xFB;	// 'Set pins o/p except bit 2 (data_in)
			}
		...
		/// <returns>Status</returns>
		private FT_STATUS SendI2cData(byte value, AcknowledgeType type = AcknowledgeType.ACK)
		{
			// Initialize ftStatus to something other than FT_OK
			FT_STATUS ftStatus = FT_STATUS.FT_OTHER_ERROR;
			...
			OutputBuffer[dwNumBytesToSend++] = value;	// Data to send

			pinData = (byte)(gpioData | 0x0E);
			// Put I2C line back to idle (during transfer) state... Clock line low, Data line high
			OutputBuffer[dwNumBytesToSend++] = 0x80;	// Command to set ADbus direction/ data
			OutputBuffer[dwNumBytesToSend++] = pinData;	// Set the value of the pins
			...
			while ((dwNumInputBuffer < 1) && (ftStatus == FT_STATUS.FT_OK) && (ReadTimeoutCounter < READ_TIMEOUT_COUNTER))
			{
				// Sit in this loop until
				// (1) we receive the one byte expected
				// or (2) a hardware error occurs causing the GetQueueStatus to return an error code
				// or (3) we have checked 'n' times and the expected byte is not coming 
				Thread.Sleep(READ_TIMEOUT_DELAY); // short delay
				ftStatus = GetRxBytesAvailable(ref dwNumInputBuffer); // 'Get the number of bytes in the device input buffer'
				ReadTimeoutCounter++;
			}

			if ((ftStatus == FT_STATUS.FT_OK) && (ReadTimeoutCounter < READ_TIMEOUT_COUNTER))
			{
				ftStatus = Read(InputBuffer, 1, ref dwNumBytesRead); // Read one byte from the device receive buffer

				if (type == AcknowledgeType.ACK)
				{
					// Check that the ACK bit is a zero and if it is not throw an error
					if ((InputBuffer[0] & 0x01) != 0x00)
					{
						ftStatus = FT_STATUS.FT_I2C_NO_ACK;
					}
				}
				else
				{
					// Check that the NACK bit is a one and if it is not throw an error
					if ((InputBuffer[0] & 0x01) != 0x01)
					{
						ftStatus = FT_STATUS.FT_I2C_NO_NACK;
					}
				}
			}
			else if (ReadTimeoutCounter >= READ_TIMEOUT_COUNTER)
			{
				ftStatus = FT_STATUS.FT_I2C_READ_TIMEOUT;
			}
		...
		/// </summary>
		/// <returns>Status</returns>
		private FT_STATUS SendI2cStopBit()
		{
			dwNumBytesToSend = 0;
			pinData = (byte)(gpioData | 0x0C);
			...
			{
				OutputBuffer[dwNumBytesToSend++] = 0x80;	// 'Command to set ADbus direction/ data
				OutputBuffer[dwNumBytesToSend++] = pinData; // 'Put data and clock low
				OutputBuffer[dwNumBytesToSend++] = 0xFB;	// 'Set pins o/p except bit 2 (data_in)
			}
			...
			{
				OutputBuffer[dwNumBytesToSend++] = 0x80;	// 'Command to set ADbus direction/ data
				OutputBuffer[dwNumBytesToSend++] = pinData; // 'Put data low, clock remains high
				OutputBuffer[dwNumBytesToSend++] = 0xFB;	// 'Set pins o/p except bit 2 (data_in)
			}
			...
			{
				OutputBuffer[dwNumBytesToSend++] = 0x80;	// 'Command to set ADbus direction/ data
				OutputBuffer[dwNumBytesToSend++] = pinData; // 'Bring both clock and data now high
				OutputBuffer[dwNumBytesToSend++] = 0xFB;	// 'Set pins o/p except bit 2 (data_in)
			}
		...
		/// </summary>
		/// <param name="value">Byte then is read in.</param>
		/// <returns>Status</returns>
		private FT_STATUS ReadByteAndSendNAK(out byte value)
		{
			...
			OutputBuffer[dwNumBytesToSend++] = pinData;		// Set the value of the pins (only affects those set as output)
			OutputBuffer[dwNumBytesToSend++] = 0xFB;		// Set the directions - all pins as output except Bit2(data_in)
		...
		///		Clock out one bit to the I2C Slave which is the ACK/NAK bit
		///		Put lines back to the idle state (idle between start and stop is clock low, data high (open-drain)
		/// This function reads only one byte from the I2C Slave. It therefore sends a '0' as the ACK/NAK bit. This is ACKing 
		/// the first byte of data, to tell the slave we want to read more bytes.
		/// </summary>
		/// <param name="value">Byte then is read in.</param>
		/// <returns>Status</returns>
		private FT_STATUS ReadByteAndSendACK(out byte value)
		{
			// Initialize the output variable
			value = 0;
			...
			OutputBuffer[dwNumBytesToSend++] = 0x13; 		// Command to clock data bits out on clock falling edge
			OutputBuffer[dwNumBytesToSend++] = 0x00;		// Length of 0x00 means clock out ONE bit
			OutputBuffer[dwNumBytesToSend++] = 0x00;		// Command will send bit 7 of this byte, we send a '0' here
		...
		private void SetupClockForMpsse(MpsseConfiguration config)
		{
			double bitRate = 0;

			// Requested frequencies are 100KHz, 400KHz, 1MHz, 1.76MHz, and 3.75MHz.
			// TODO - Pass in the actual value instead of the selection
			switch(config.ClockSelection)
			{
				case 0:
					bitRate = 100000.0;
					break;
			...
			// Set high speed flag if greater than 1MHz
			isHighSpeed = (bitRate > 1000000) ? true : false;

			// Get divisor for bitRate
			double d = CalculateClkDivisor(bitRate);

			// If three phase clocking is enabled than increase rates by 2/3
			if (config.GetPhaseClocking() == 0x8C)
			{
				d *= (2.0 / 3.0);
				clockRate = (ushort)d;

				d = CalculateClkDivisor(400000);
				d *= (2.0 / 3.0);
				fastSpeedClock = (ushort)d;
			}
			else
			{
				clockRate = (ushort)d;
				fastSpeedClock = (ushort)CalculateClkDivisor(400000);
			}
		}

		/// <summary>
		/// Calculates the clock divider.
		/// </summary>
		/// <param name="bitRate">The desired clock frequency</param>
		/// <returns>Clock divider.</returns>
		private double CalculateClkDivisor(double bitRate)
		{
			// Enforce max bit rate of 30MHz and min 457.763Hz
			bitRate = (bitRate > 30000000) ? 30000000 : bitRate;
			bitRate = (bitRate < 457.763) ? 457.763 : bitRate;

			// Return the divisor
			return ((60000000.0 / bitRate) / 2.0) - 1.0;
		}
		...
		public FT_STATUS GetNumberOfDevices(ref UInt32 devcount)
		{
			...
			{
				throw new FT_EXCEPTION("Failed to load function FT_CreateDeviceInfoList.");
			}
			return ftStatus;
		...
		public FT_STATUS GetDeviceList(FT_DEVICE_INFO_NODE[] devicelist)
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetDeviceInfoListDetail.");
				}
			}
			return ftStatus;
		}
		...
		/// <remarks>Initializes the device to 8 data bits, 1 stop bit, no parity, no flow control and 9600 Baud.</remarks>
		public FT_STATUS OpenByIndex(UInt32 index)
		{
			...
			if ((pFT_Open != IntPtr.Zero) & (pFT_SetDataCharacteristics != IntPtr.Zero) & (pFT_SetFlowControl != IntPtr.Zero) & (pFT_SetBaudRate != IntPtr.Zero))
			{
				...
				{
					// Initialize port data characteristics
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_Open.");
				}
				if (pFT_SetDataCharacteristics == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetDataCharacteristics.");
				}
				if (pFT_SetFlowControl == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetFlowControl.");
				}
				if (pFT_SetBaudRate == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetBaudRate.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS OpenBySerialNumber(string serialnumber)
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_OpenEx.");
				}
				if (pFT_SetDataCharacteristics == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetDataCharacteristics.");
		...
		public FT_STATUS Close()
		{
			...
			else
			{
				if (pFT_Close == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_Close.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Read(byte[] dataBuffer, UInt32 numBytesToRead, ref UInt32 numBytesRead)
		{
			...
			else
			{
				if (pFT_Read == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_Read.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Write(byte[] dataBuffer, Int32 numBytesToWrite, ref UInt32 numBytesWritten)
		{
			...
			else
			{
				if (pFT_Write == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_Write.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ResetDevice()
		{
			...
			else
			{
				if (pFT_ResetDevice == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_ResetDevice.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Purge(UInt32 purgemask)
		{
			...
			else
			{
				if (pFT_Purge == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_Purge.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetEventNotification(UInt32 eventmask, EventWaitHandle eventhandle)
		{
			...
			else
			{
				if (pFT_SetEventNotification == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetEventNotification.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS StopInTask()
		{
			...
			else
			{
				if (pFT_StopInTask == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_StopInTask.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS RestartInTask()
		{
			...
			else
			{
				if (pFT_RestartInTask == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_RestartInTask.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ResetPort()
		{
			...
			else
			{
				if (pFT_ResetPort == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_ResetPort.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS CyclePort()
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_CyclePort.");
				}
				if (pFT_Close == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_Close.");
		...
		public FT_STATUS Rescan()
		{
			...
			else
			{
				if (pFT_Rescan == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_Rescan.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Reload(UInt16 VendorID, UInt16 ProductID)
		{
			...
			else
			{
				if (pFT_Reload == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_Reload.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetBitMode(byte Mask, byte BitMode)
		{
			...
			else
			{
				if (pFT_SetBitMode == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetBitMode.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetPinStates(ref byte BitMode)
		{
			...
			else
			{
				if (pFT_GetBitMode == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetBitMode.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ReadEEPROMLocation(UInt32 Address, ref UInt16 EEValue)
		{
			...
			else
			{
				if (pFT_ReadEE == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_ReadEE.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS WriteEEPROMLocation(UInt32 Address, UInt16 EEValue)
		{
			...
			else
			{
				if (pFT_WriteEE == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_WriteEE.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS EraseEEPROM()
		{
			...
			else
			{
				if (pFT_EraseEE == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_EraseEE.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ReadFT232BEEPROM(FT232B_EEPROM_STRUCTURE ee232b)
		{
			...
			else
			{
				if (pFT_EE_Read == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_EE_Read.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS WriteFT232BEEPROM(FT232B_EEPROM_STRUCTURE ee232b)
		{
			...
			else
			{
				if (pFT_EE_Program == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_EE_Program.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS EEReadUserArea(byte[] UserAreaDataBuffer, ref UInt32 numBytesRead)
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_EE_UASize.");
				}
				if (pFT_EE_UARead == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_EE_UARead.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS EEWriteUserArea(byte[] UserAreaDataBuffer)
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_EE_UAWrite.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetDeviceType(ref FT_DEVICE DeviceType)
		{
			...
			else
			{
				if (pFT_GetDeviceInfo == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetDeviceInfo.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetRxBytesAvailable(ref UInt32 RxQueue)
		{
			...
			else
			{
				if (pFT_GetQueueStatus == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetQueueStatus.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetTxBytesWaiting(ref UInt32 TxQueue)
		{
			...
			else
			{
				if (pFT_GetStatus == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetStatus.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetModemStatus(ref byte ModemStatus)
		{
			...
			else
			{
				if (pFT_GetModemStatus == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetModemStatus.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetRTS(bool Enable)
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetRts.");
				}
				if (pFT_ClrRts == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_ClrRts.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetDTR(bool Enable)
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetDtr.");
				}
				if (pFT_ClrDtr == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_ClrDtr.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetTimeouts(UInt32 ReadTimeout, UInt32 WriteTimeout)
		{
			...
			else
			{
				if (pFT_SetTimeouts == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetTimeouts.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetBreak(bool Enable)
		{
			...
			else
			{
				...
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetBreakOn.");
				}
				if (pFT_SetBreakOff == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetBreakOff.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetResetPipeRetryCount(UInt32 ResetPipeRetryCount)
		{
			...
			else
			{
				if (pFT_SetResetPipeRetryCount == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetResetPipeRetryCount.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetDriverVersion(ref UInt32 DriverVersion)
		{
			...
			else
			{
				if (pFT_GetDriverVersion == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetDriverVersion.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetLibraryVersion(ref UInt32 LibraryVersion)
		{
			...
			else
			{
				if (pFT_GetLibraryVersion == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetLibraryVersion.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetDeadmanTimeout(UInt32 DeadmanTimeout)
		{
			...
			else
			{
				if (pFT_SetDeadmanTimeout == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetDeadmanTimeout.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetLatency(byte Latency)
		{
			...
			else
			{
				if (pFT_SetLatencyTimer == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetLatencyTimer.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetLatency(ref byte Latency)
		{
			...
			else
			{
				if (pFT_GetLatencyTimer == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetLatencyTimer.");
				}
			}
			return ftStatus;
		}
		...
		//public UInt32 InTransferSize(UInt32 InTransferSize, UInt32 OutTransferSize)
		{
			...
			else
			{
				if (pFT_SetUSBParameters == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetUSBParameters.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetCharacters(byte EventChar, bool EventCharEnable, byte ErrorChar, bool ErrorCharEnable)
		{
			...
			else
			{
				if (pFT_SetChars == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_SetChars.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetCOMPort(out string ComPortName)
		{
			...
			else
			{
				if (pFT_GetComPortNumber == IntPtr.Zero)
				{
					throw new FT_EXCEPTION("Failed to load function FT_GetComPortNumber.");
				}
			}
			return ftStatus;
		}
